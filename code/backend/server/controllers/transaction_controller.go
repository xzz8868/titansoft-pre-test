...
func (t *TransactionController) GetTransactionsByCustomer(ctx echo.Context) error {
    ...
    transactions, err := t.service.GetTransactionsByCustomer(customerID, from, to)
    if err != nil {
...
func (t *TransactionController) GetTransactionByID(ctx echo.Context) error {
    id, err := uuid.Parse(ctx.Param("id"))
    if err != nil {
        return ctx.JSON(http.StatusBadRequest, "Invalid ID")
    }
    transaction, err := t.service.GetTransactionByID(id)
    if err != nil {
